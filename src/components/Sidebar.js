  ...
  setTime = (e, item, maxMin, minMin, maxSec, minSec) => {
    ...
    if (e.key === 'ArrowUp') {
      if (secundes > 40) {
        secundes = 0;
        minutes++;
      } else if (minutes >= maxMin && secundes >= maxSec) { //maximum
        minutes = maxMin;
        secundes = maxSec;
      }
      else secundes+=10;
    } else if (e.key === 'ArrowDown') {
      if (minutes === minMin && secundes === minSec) { //minimum
        minutes = minMin;
        secundes = minSec;
      }
      else if (secundes <= 0 ) {
  ...
  render() {
    ...
    return (
      <div className="container" id="sidebar">
        ...
        <div className="blocks">
          Round Time <input type="text" id="input4" value = {roundTime} onChange = {this.handleInputs} onKeyDown = {(e) => {
            let value = this.setTime(e,roundTime, 10,0,0,10);
            this.setState({roundTime: value});
          }}  />
        </div>
        <div className="blocks">
          Rest Time <input type="text" id="input5" value={restTime} onChange = {this.handleInputs} onKeyDown = {(e) => {
            let value = this.setTime(e,restTime, 10,0,0,10);
            this.setState({restTime: value});
          }}/>
        </div>
        <div className="blocks">
          Alerter <input type="text" id="input2" value={alerter} onChange = {this.handleInputs} onKeyDown = {(e) => {
            let value = this.setTime(e,alerter,1,0,0,0);
            this.setState({alerter: value});
          }}/>
        </div>
        <div className="blocks">
          Prepare <input type="text" id="input3" value= {prepareTime} onChange = {this.handleInputs} onKeyDown = {(e) => {
